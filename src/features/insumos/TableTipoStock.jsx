  ...
  return (
    <section className="card p-4 overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map(t => (
            <tr key={t.id} className="border-t" style={{ borderColor: "var(--frame)" }}>
              <td className="py-2">{t.nombre}</td>
              <td className="py-2">{t.activo ? "Activo" : "Inactivo"}</td>
              <td className="py-2">
                <div className="flex gap-2 flex-wrap">
                  <button className="btn btn-outline" onClick={() => onEditar(t.id)}>Modificar</button>
                  <button className="btn btn-outline" onClick={() => onEliminar(t.id)}>Eliminar</button>
                </div>
              </td>
            </tr>
          ))}
